...
class UI(QMainWindow):
    ...
    def results(self):
        if self.pixmap:
            ...
            model = keras.models.load_model('Model/ResNet50Varieties30.h5')
            # print("hello")
            ...
            self.bousselam.setText(str(np.count_nonzero(predictions == 0)))
            self.gta.setText(str(np.count_nonzero(predictions == 1)))
            self.oued_el_bared.setText(str(np.count_nonzero(predictions == 2)))
            self.vitron.setText(str(np.count_nonzero(predictions == 3)))
